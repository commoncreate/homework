...
bool TileMap::generateArenaLevel(const std::string& tilesetPath, unsigned ts) {
    tileSize = ts;
    sf::Image img;
    img.create(64, 16, sf::Color::Transparent);
    for (unsigned x = 0; x < 16; ++x)
        for (unsigned y = 0; y < 16; ++y) {
            sf::Color c = (y < 3) ? sf::Color(100, 200, 80)  
                : sf::Color(60, 130, 50);  
            img.setPixel(x, y, c);
        }
    for (unsigned x = 16; x < 32; ++x)
        for (unsigned y = 0; y < 16; ++y) {
            sf::Color c = ((x + y) % 4 == 0) ? sf::Color(110, 75, 45)
                : sf::Color(140, 95, 55);
            img.setPixel(x, y, c);
        }
    ...
    for (unsigned x = 32; x < 48; ++x)
        for (unsigned y = 0; y < 16; ++y) {
            sf::Color c = ((x + y) % 3 == 0) ? sf::Color(70, 70, 80)
                : sf::Color(100, 100, 110);
            img.setPixel(x, y, c);
        }

    for (unsigned x = 48; x < 64; ++x)
        for (unsigned y = 0; y < 16; ++y) {
            img.setPixel(x, y, sf::Color(50, 40, 60));
        }

    for (unsigned block = 0; block < 4; ++block) {
        for (unsigned x = 0; x < 16; ++x) {
            img.setPixel(block * 16 + x, 0, sf::Color(20, 20, 20));
            img.setPixel(block * 16 + x, 15, sf::Color(20, 20, 20));
        }
        for (unsigned y = 0; y < 16; ++y) {
            img.setPixel(block * 16 + 0, y, sf::Color(20, 20, 20));
            img.setPixel(block * 16 + 15, y, sf::Color(20, 20, 20));
        }
    }
    ...
    std::cout << "     Using procedural tileset (no PNG needed)\n";
    return true;
}
